defmodule PetroMindWeb.ChatLive.FormComponent do
  ...
  def update(%{chat: chat} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Dashboard.change_chat(chat))
     end)}
  end

  @impl true
  def handle_event("validate", %{"chat" => chat_params}, socket) do
    changeset = Dashboard.change_chat(socket.assigns.chat, chat_params)
  ...
  defp save_chat(socket, :edit, chat_params) do
    case Dashboard.update_chat(socket.assigns.chat, chat_params) do
      {:ok, chat} ->
  ...
  defp save_chat(socket, :new, chat_params) do
    case Dashboard.create_chat(chat_params) do
      {:ok, chat} ->
        notify_parent({:saved, chat})
